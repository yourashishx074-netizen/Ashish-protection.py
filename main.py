...
def start_buttons():
    return [
        [Button.url("✨ 𝐓ᴀᴘ 𝐓ᴏ 𝐒ᴇᴇ 𝐌ᴀɢɪᴄ ✨", MAGIC_URL)],
        [Button.inline("𝐇ᴇʟᴘ 🛠️", b"open_help"), Button.url("𝐒ᴜᴘᴘᴏʀᴛ 𝐆ᴄ 💬", "https://t.me/Rinnegan_anime_group")],
        [Button.url("𝐎ᴡɴᴇʀ 👑", f"https://t.me/{OWNER_USERNAME}")],
        [Button.inline("➕ 𝐋ᴏɢɪɴ 𝐔sᴇʀʙᴏᴛ", b"start_login")]
    ]

HELP_TEXT = f"""{header()}

🔥 RAID & SPAM
.raid [COUNT]       Target ke reply pe fast raid (Mention + Reply)
.rraid              Reply to target for continuous auto-raid
.drraid             Deactivate continuous reply raid
.oneraid [COUNT/TXT] One-word sequential raid (Mention + Reply)
.spam [COUNT] [TXT] Spam — Reply optional (Mention + Reply if replied)
...
.scan               Safe system scan
.launch             Rocket launch
.matrix             Matrix animation
.hack               Fake harmless terminal
.breakdown TEXT     Words ko line by line todna
...
.back               Restore your original profile
...
def build_welcome_message(first_name: str):
    ...
    part_footer = "✨ 𝐓ᴀᴘ 𝐓ʜᴇ 𝐁ᴜᴛᴛᴏɴs 𝐁ᴇʟᴏᴡ 𝐀ɴᴅ 𝐄xᴘʟᴏʀᴇ 𝐓ʜᴇ 𝐌ᴀɢɪᴄ!"
...
async def start_handler(event):
    if not await is_subscribed(event.sender_id):
        join_btn = [
            ...
            [Button.url("🔄 ʀᴇᴛʀʏ", f"https://t.me/{(await bot.get_me()).username}?start=start")]
        ]
        await event.respond(
            FORCE_JOIN_TEXT,
    ...
    full_text, entities = build_welcome_message(first)
...
async def callback_back(event):
    ...
    try:
        msg = await event.get_message()
        if msg and msg.media:
            await event.edit(text=full_text, formatting_entities=entities, buttons=start_buttons())
        else:
            await event.edit(full_text, formatting_entities=entities, buttons=start_buttons())
    except Exception:
        try:
            await event.delete()
        except Exception:
...
async def broadcast_handler(event):
    ...
    if not reply and not text:
        await event.respond("⚠️ Kisi message ko reply karke `/broadcast` likho ya `/broadcast <text>` bhejo.")
        return
...
def register_userbot_handlers(uclient: TelegramClient, owner_uid: int):
    ...
    async def oneraid_cmd(event):
        reply = await event.get_reply_message()
        if not reply or not reply.sender_id:
            await reply_card(uclient, event, "⚠️ Target ke message ko reply karke `.oneraid` lagao.")
            return
        ...
        if args and not args.strip().isdigit():
            word_sequence = args.strip().split()
        else:
            limit = int(args.strip()) if (args and args.strip().isdigit()) else len(ONE_WORD_RAID_TEXTS)
